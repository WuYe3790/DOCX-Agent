...
MessageManager 模块自动管理消息累积和去重：

- 内部使用追加模式，通过两遍倒序扫描实现自动去重
- 去重范围：`write_markdown_draft` 和 `read_markdown_draft`（同一文件的多次写入只保留最新）
- 配对删除：tool call 和其 tool result 一起删除，防止模型困惑
- Token 追踪：累计 `prompt_tokens` 并通过 Header 进度条实时显示
...
├── context_manager.py         # 消息管理模块（自动去重 + token 追踪）
...
├── docx_tools/               # DOCX 编辑工具（28+ 个）
│   ├── registry.py           # TOOLS_SCHEMA 和 call_tool
│   ├── common.py            # 共享 XML/ZIP 工具
│   ├── analyze_docx_style_samples.py
...
│   ├── diagnostics.py        # 诊断系统
│   └── table_ops.py          # 表格操作
├── 文档格式测试/               # 测试用例与 before/after DOCX 对
├── out/                       # 输出目录
└── logs/                      # 运行日志
